use std::{borrow::{Borrow, BorrowMut}, collections::{HashMap, LinkedList}};
...
pub(crate) struct Board {
    ...
    pub board_liberties: HashMap<String, LinkedList<(u8,  u8)>>
...
impl Board {
    ...
    pub fn place_stone(
    &mut self, player: &Player, row: usize, col: usize) -> bool {
        if self.board_state[row][col].player_color == 0 {
            self.board_state[row][col].player_color = player.player_color;
            return true;
        } else {
            return false;
        }
    ...
    pub fn get_friends(&self, intersection: Intersection) -> LinkedList<&Intersection> {
        //Check left, right, above and below for "friendly" intersections
        let mut friends: LinkedList<&Intersection> = LinkedList::new();
    
        //Check left friend
        if intersection.col != 0 {
            if self.board_state[intersection.row][intersection.col-1].player_color == intersection.player_color {
                friends.push_back(&self.board_state[intersection.row][intersection.col-1]);
            }
        }

        //Check right friend
        if intersection.col+1 != self.board_size {
            if self.board_state[intersection.row][intersection.col+1].player_color == intersection.player_color {
                friends.push_back(&self.board_state[intersection.row][intersection.col+1]);
            }
        }
        ...
        if intersection.row != 0 {
            if self.board_state[intersection.row+1][intersection.col].player_color == intersection.player_color {
                friends.push_back(&self.board_state[intersection.row+1][intersection.col]);
            }
        }

        //Check friend below
        if intersection.row-1 != self.board_size {
            if self.board_state[intersection.row-1][intersection.col].player_color == intersection.player_color {
                friends.push_back(&self.board_state[intersection.row-1][intersection.col]);
            }
        }
    ...
    //Check for surrounding area to identify if its been added to a chain
    pub fn check_for_chain(&mut self, intersection: Intersection) {
        
        let friends = self.get_friends(intersection);
        //All of these friends needs to be made into a single chain
        //They will now have the id of the newly placed stone
        for friend in friends {
            if let Some(links) = self.board_chains.get(&friend.chain_id) {
                for link in links {
                    //self.board_state[link.0][link.1].chain_id = intersection.chain_id.clone();
                }
            }
        }
...
mod tests {
    ...
    fn test_place_stone() {
        ...
        assert_eq!(test_board.board_state[2][2].player_color, crate::game::BLACK);
        let result2 = test_board.place_stone(&test_player, 8, 8);
        let result3 = test_board.place_stone(&test_player, 8, 8);
        assert_eq!(test_board.board_state[8][8].player_color, crate::game::BLACK);
    ...
    fn test_check_for_capture() {
        ...
        test_board.place_stone(&test_white, 0, 0);
        test_board.place_stone(&test_white, 0, 1);
        test_board.place_stone(&test_white, 0, 2);
        test_board.place_stone(&test_white, 0, 3);
        test_board.place_stone(&test_white, 1, 0);
        test_board.place_stone(&test_black, 1, 1);
        test_board.place_stone(&test_black, 1, 2);
        //test_board[2][1] getting acquired by white (1) will capture the black (2) at [1][1]
        test_board.place_stone(&test_white, 2, 1);
